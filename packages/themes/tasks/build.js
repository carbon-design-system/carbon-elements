...
const THEME_ENTRYPOINT = path.join(SCSS_DIR, '_theme.scss');
...
async function build() {
  ...
  let defaultThemeMap = `${defaultThemeMapName}: (\n`;
  for (const key of Object.keys(themes.white)) {
    const name = formatTokenName(key);
    const value = themes.white[key];
    defaultThemeMap += `  ${name}: ${value},`;
    defaultThemeMap += '\n';
  }
  defaultThemeMap += ') !default !global;';
...
${themeMixinAlias}`;
  ...
  await fs.writeFile(THEME_ENTRYPOINT, prettier.format(theme, prettierOptions));
  await fs.writeFile(
